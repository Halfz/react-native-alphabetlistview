...
export default class SelectableSectionsListView extends React.Component {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      UIManager.measure(ReactNative.findNodeHandle(this.references.view), (x, y, w, h) => {
  ...
  scrollToSection(section) {
    ...
    if (!this.props.useDynamicHeights) {
      ...
      this.references.listview.scrollTo({x: 0, y, animated: true});
    } else {
      UIManager.measureLayout(this.cellTagMap[section], ReactNative.findNodeHandle(this.references.listview), () => {
      }, (x, y, w, h) => {
